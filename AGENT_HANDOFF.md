...
- **本轮（2026-09-11 第八轮：在途三组产物收口 + UI-16-B 两批 + 私服链路加固，主会话）**：
  ...
  - **UI-16-B 第三/四批 done（f4bfa04 + d9039dd）**：第三批 ExecutorPackagesPage（listPackages 失败 → StateError；loadSeq 竞态守卫保证快速重入/卸载时只最新请求可写状态）+ 4 例；第四批补 3 个 P2 页——**NotificationSettingsPage**（渠道列表失败且无缓存 → StateError，替代 Card loading 后空白；静默规则列表失败 → StateError 不落「暂无静默规则」）、**settings/index**（执行器 Token / 系统配置 / 变更历史 / AI 配置四处读请求失败各接 StateError+重试，此前为永久 Spin / 空表 / 空白 Modal / 永久 Spin）、**DashboardPage**（summary/trend/executors/failures 任一失败 → 整页 StateError + 四路 refetch，不再以骨架/空数字掩盖失败）+ 5 例（`ui16-batch4-error-state.test.tsx`）。UI-16-B 累计 9 页；**剩余 4 页**=ExecutorInstallWizard / settings.ApiKeys / settings.EventSubscriptions / settings.Security。统一约定：只覆盖读请求失败，写操作继续 toast（UI-15）。
  ...
  - **基线（本轮收口）**：admin-web **639/639**（71 文件，基线 605，+34）· executor-node **273/273** · executor-python **247/247** · 私服契约自检 22/22 · 双端 tsc 绿 · lint 0 error / 21 既有 warning。

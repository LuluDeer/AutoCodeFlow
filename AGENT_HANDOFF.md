...
- **里程碑（2026-09-05 Windows 轮，R13-R16 全完成，Windows 侧接手主导）**：项目首个非 Linux 平台全验证（Win11 26200 / Node 24.17 / Python 3.12-uv / WSL2 mirrored 网络跑 PG16+Redis7）。findings **W-01~W-20**、生产修复 **P-1~P-15**（`docs/windows-findings.md`）：
  ...
  - **双平台绿灯基线**：executor-node 162/162（3 连跑稳）、executor-python **127/127 零 skip**、admin-api 873/873 + eslint 0/0、acf-cli 48、mcp-server 61、registry-pypi 33、admin-web 35 + e2e **16/16**（16 例版；**W-12：任务书 29 例系 Linux 未跟踪文件，需入库**）

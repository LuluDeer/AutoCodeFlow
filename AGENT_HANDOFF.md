...
- **本轮（2026-09-11 第八轮：在途三组产物收口 + UI-16-B 两批 + 私服链路加固，主会话）**：
  ...
  - **下轮建议**：① UI-16-B 收尾 4 页（纯前端可本机验证）；② UI-09 真实 375px 浏览器验收；③ QA-05/BUG-18/ARCH-31 真机与真 PostgreSQL 验证；④ e2e 例 23/24（QA-01）；⑤ AUTH-02（依赖 AUTH-01 拍板）；⑥ **develop→main 合并 + release PR #2（release-please，OPEN/MERGEABLE/UNSTABLE）待处置**（main 落后 develop 43+ commit；合并会触发 v* tag → release.yml 三包发布，需 environment 审批与 NPM_TOKEN/PYPI_API_TOKEN）。

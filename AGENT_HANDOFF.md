...
- **本轮（2026-09-11 第九轮：主干合并 + 1.1.1 发布 + 真机验证，主会话）**：
  ...
  - **v1.1.1 发布推进（编译/守卫全绿，卡在人工审批闸）**：
    ① release-please 产出 Release PR #3（三包 → 1.1.1）；② **发现 linked-versions 只联动「本轮有候选发布的组件」**——node-sdk 无路径内提交被跳过，会在 tag 时被 version-guard 拦（fail 安全）→ 在发布 PR 内补齐 node-sdk（package.json/lock/CHANGELOG/manifest）+ **DOC-09 版本声明同步**（源头 sdk-guide + 站点 6 页，1.1.0→1.1.1）；③ 合并 `ee3f0c8` → release-please 打 **tag v1.1.1**；④ `release.yml` run 34576901825：**version-guard success**，3 个 publish job `waiting`（`environment: release` 人工审批，**等用户 Approve**）。
  ...
  - **基线与状态**：develop == main == `ee3f0c8`（+ `4cbdffc` 已在 develop）；四源版本 1.1.1 · sync-check 七面绿 · mcp 100/100 · node-sdk 63/63 · py-sdk 111/111 · registry-pypi 68/68。
  - **唯一阻塞项（需用户动作）**：Approval `release` environment → https://github.com/LuluDeer/AutoCodeFlow/actions/runs/34576901825 （Approve 后三包自动发布；建议随后做安装冒烟：`npm view autocodeflow-mcp-server version` / `npx -y autocodeflow-mcp-server --version` / `pip install autoflow-sdk==1.1.1`）。

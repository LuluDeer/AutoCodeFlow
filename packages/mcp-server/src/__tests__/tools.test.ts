...
const registerFns = [registerTaskTools, registerApplicationTools, registerDeploymentTools, registerExecutorTools, registerAuditTools];

function setup(): { tools: Map<string, RegisteredTool>; call: ReturnType<typeof vi.fn> } {
...
describe('tool registry surface', () => {
  it('registers the full P1-aligned capability face', () => {
    const expected = [
      ...
      'delete_application', 'analyze_application',
      // deployments
...
describe('list_tasks', () => {
  it('GETs /tasks with page/pageSize and optional name/status', async () => {
    await tools.get('list_tasks')!.handler({ page: 2, pageSize: 10, name: 'demo', status: 'active' });
...
describe('get_task / trigger_task', () => {
  ...
  it('trigger_task POSTs /tasks/:id/trigger with params only (no executorId)', async () => {
    await tools.get('trigger_task')!.handler({ taskId: 't1', params: { a: 1 } });
    expect(call).toHaveBeenCalledWith('POST', '/tasks/t1/trigger', { params: { a: 1 } });
  });
...
describe('rollback_task_version', () => {
  it('POSTs /tasks/:id/versions/:versionId/rollback', async () => {
    await tools.get('rollback_task_version')!.handler({ taskId: 't1', versionId: 'v9' });
...
describe('compare_task_versions', () => {
  it('GETs /tasks/:id/versions/:v1/compare/:v2', async () => {
    await tools.get('compare_task_versions')!.handler({ taskId: 't1', versionId1: 'a', versionId2: 'b' });
...
describe('execution tools', () => {
  it('list_executions GETs /tasks/executions/all with filters', async () => {
    await tools.get('list_executions')!.handler({ taskId: 't1', status: 'failed', page: 1, pageSize: 10 });
    const [method, path] = call.mock.calls[0];
    expect(method).toBe('GET');
    expect(path).toBe('/tasks/executions/all?page=1&pageSize=10&taskId=t1&status=failed');
  });
  ...
  it('get_execution_logs GETs /tasks/executions/:id/logs with fromLine/limit', async () => {
    await tools.get('get_execution_logs')!.handler({ executionId: 'e1', fromLine: 5, limit: 100 });
    expect(call).toHaveBeenCalledWith('GET', '/tasks/executions/e1/logs?fromLine=5&limit=100');
  });

  it('kill_execution POSTs the task-scoped kill route', async () => {
    await tools.get('kill_execution')!.handler({ taskId: 't1', executionId: 'e1' });
...
describe('create_application', () => {
  it('POSTs all provided fields to /applications', async () => {
    await tools.get('create_application')!.handler({
      name: 'demo', version: '1.0.0', runtime: 'node',
      description: 'd', env: { K: 'V' }, manifest: { a: 1 },
    });
...
describe('update_application', () => {
  it('PUTs the patch to /applications/:id', async () => {
    await tools.get('update_application')!.handler({ applicationId: 'a1', version: '2.0.0', status: 'INACTIVE' });
    expect(call).toHaveBeenCalledWith('PUT', '/applications/a1', { version: '2.0.0', status: 'INACTIVE' });
  });
  ...
  it('drops undefined fields (forbidNonWhitelisted rejects unknown properties)', async () => {
    await tools.get('update_application')!.handler({
      applicationId: 'a1', version: '2.0.0', description: undefined,
    });
    expect(call).toHaveBeenCalledWith('PUT', '/applications/a1', { version: '2.0.0' });
  });
...
describe('deploy_application', () => {
  it('POSTs /app-deployments/applications/:id/deploy with only the provided fields', async () => {
    await tools.get('deploy_application')!.handler({
      applicationId: 'a1', executorId: 'e1', runMode: 'daemon',
    });
    expect(call).toHaveBeenCalledWith('POST', '/app-deployments/applications/a1/deploy', {
      executorId: 'e1', runMode: 'daemon',
    });
  });
});
...
describe('list_deployments', () => {
  it('GETs /app-deployments with applicationId filter', async () => {
    await tools.get('list_deployments')!.handler({ applicationId: 'a1', page: 1, pageSize: 20 });
    expect(call).toHaveBeenCalledWith('GET', '/app-deployments?page=1&pageSize=20&applicationId=a1');
  });
});
...
describe('list_audit_logs', () => {
  it('GETs /audit with the AuditQueryDto whitelist params', async () => {
    await tools.get('list_audit_logs')!.handler({
      page: 2, pageSize: 50, action: 'task.trigger', resource: 'task',
      userId: 3, username: 'admin', startTime: '2026-01-01T00:00:00Z', endTime: '2026-02-01T00:00:00Z',
    });
    const [method, path] = call.mock.calls[0];
    expect(method).toBe('GET');
    const url = new URL(path, 'http://x');
...
describe('tool handler output', () => {
  it('returns JSON text content with the apiRequest result', async () => {
    ...
    const result = await tools.get('list_tasks')!.handler({ page: 1, pageSize: 20 });

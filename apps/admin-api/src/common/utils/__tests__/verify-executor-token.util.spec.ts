...
describe("verifyExecutorToken", () => {
  describe("when no token is configured", () => {
    it("throws in development even without auth header (fail closed)", async () => {
      ...
      await expect(
        verifyExecutorToken(undefined, cfg as any, sys as any),
    ...
    it("throws in test even with a token in header (fail closed)", async () => {
      ...
      const sys = makeSystemConfig(null);
      await expect(
        verifyExecutorToken("any-token", cfg as any, sys as any),
      ).rejects.toBeInstanceOf(UnauthorizedException);
    });
    ...
    it("throws in production with a token in header when none configured", async () => {
      ...
      const sys = makeSystemConfig(null);
      await expect(
        verifyExecutorToken("Bearer anything", cfg as any, sys as any),
      ).rejects.toBeInstanceOf(UnauthorizedException);
    });
  });
  ...
  describe("when token is configured via env/config", () => {
    it("accepts matching Bearer token", async () => {
      ...
      const sys = makeSystemConfig(null);
      await expect(
        verifyExecutorToken(`Bearer ${SECRET}`, cfg as any, sys as any),
    ...
    it("accepts raw token (no Bearer prefix)", async () => {
      ...
      const sys = makeSystemConfig(null);
      await expect(
        verifyExecutorToken(SECRET, cfg as any, sys as any),
      ).resolves.toBeUndefined();
    });
    ...
    it("rejects wrong token", async () => {
      ...
      const sys = makeSystemConfig(null);
      await expect(
        verifyExecutorToken("Bearer wrong-token", cfg as any, sys as any),
      ).rejects.toBeInstanceOf(UnauthorizedException);
    });
    ...
    it("rejects empty auth header when token is set", async () => {
      ...
      const sys = makeSystemConfig(null);
      await expect(
        verifyExecutorToken("", cfg as any, sys as any),
      ).rejects.toBeInstanceOf(UnauthorizedException);
    });
    ...
    it("rejects token that is superset of the actual token", async () => {
      ...
      const sys = makeSystemConfig(null);
      await expect(
        verifyExecutorToken(`Bearer ${SECRET}X`, cfg as any, sys as any),
      ).rejects.toBeInstanceOf(UnauthorizedException);
    });
  });
  ...
  describe("when DB token overrides env token", () => {
    it("accepts DB token and rejects env token", async () => {
      ...
      // DB token works
      await expect(
        verifyExecutorToken(`Bearer ${dbToken}`, cfg as any, sys as any),
      ).resolves.toBeUndefined();
    });
    ...
    it("DB findOne failure falls back to env token", async () => {
      ...
      const sys = {
        findOne: jest.fn().mockRejectedValue(new Error("db offline")),
      };
      await expect(
        verifyExecutorToken(`Bearer ${SECRET}`, cfg as any, sys as any),
      ).resolves.toBeUndefined();

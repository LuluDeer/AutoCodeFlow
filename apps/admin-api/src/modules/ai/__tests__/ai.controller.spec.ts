...
describe('AiController (N11)', () => {
  ...
  beforeEach(() => {
    ...
    controller = new AiController(aiSvc as unknown as AiService, sysCfg as unknown as SystemConfigService);
  });
  ...
  describe('RolesGuard semantics', () => {
    ...
    it('plain user is denied on GET/POST /ai/config (403)', () => {
      expect(
        guard.canActivate(ctxWith(AiController.prototype.getConfig, UserRole.USER)),
      ).toBe(false);
      expect(
        guard.canActivate(ctxWith(AiController.prototype.saveConfig, UserRole.USER)),
      ).toBe(false);
    });

    it('admin passes on GET/POST /ai/config (200 path)', () => {
      expect(
        guard.canActivate(ctxWith(AiController.prototype.getConfig, UserRole.ADMIN)),
      ).toBe(true);
      expect(
        guard.canActivate(ctxWith(AiController.prototype.saveConfig, UserRole.ADMIN)),
      ).toBe(true);
    });
  });

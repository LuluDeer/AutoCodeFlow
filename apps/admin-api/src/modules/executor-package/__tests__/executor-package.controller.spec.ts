...
describe("ExecutorPackageController download HTTP contract", () => {
  ...
  const pkg = { id, name: "runner", version: "1.0.0", originalFilename: "runner.zip" };
  ...
  beforeAll(async () => {
    const module = await Test.createTestingModule({
      ...
      providers: [
        ...
        { provide: ExecutorService, useValue: { findAll: jest.fn().mockResolvedValue([
  ...
  beforeEach(() => {
    ...
    getFileBuffer.mockResolvedValue({ buffer, pkg });
  });
  afterAll(async () => { await app.close(); });

  it("downloads file bytes with the DB shared token", async () => {
    const response = await request(app.getHttpServer()).get(path)
      .set("Authorization", "Bearer db-token").expect(200);
    expect(response.body).toEqual(buffer);
    expect(response.headers["content-disposition"]).toBe('attachment; filename="runner.zip"');
  ...
  it("preserves administrator access JWT downloads", async () => {
    const token = jwt.sign({ sub: 1, username: "admin", type: "access" }, secret, { expiresIn: "15m" });
    const response = await request(app.getHttpServer()).get(path)
  ...
  it.each([
    ...
    ["expired", `Bearer ${jwt.sign({ type: "access" }, secret, { expiresIn: -1 })}`],
    ["wrong signature", `Bearer ${jwt.sign({ type: "access" }, "other-secret")}`],
  ])("rejects %s credentials before reading file bytes", async (_name, header) => {
    const req = request(app.getHttpServer()).get(path);
    if (header) req.set("Authorization", header);
    const response = await req.expect(401);
    expect(response.body.message).toBe("Unauthorized: package downloads require a valid access JWT or executor token");
  ...
  it("downloads the URL generated by the push payload using its DB credential", async () => {
    ...
    const response = await request(app.getHttpServer()).get(new URL(payload.downloadUrl).pathname)
      .set("Authorization", options.headers.Authorization).expect(200);
    expect(response.body).toEqual(buffer);
  });
});

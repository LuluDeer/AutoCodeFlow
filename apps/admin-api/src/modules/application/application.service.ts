import { Injectable, Logger, NotFoundException, ConflictException, Inject, forwardRef, OnModuleInit } from '@nestjs/common';
...
export class ApplicationService implements OnModuleInit {
  ...
  async onModuleInit() {
    ...
    try {
      ...
      this.logger.warn('TaskService not available — manifest auto-registration disabled');
    }
  }
  ...
  async create(dto: CreateApplicationDto): Promise<Application> {
    ...
    const app = this.repo.create({
      ...dto,
      status: dto.gitRepo ? ApplicationStatus.DEPLOYING : ApplicationStatus.ACTIVE,
    });
    ...
    if (dto.gitRepo) {
      this.deployFromGit(saved.id, dto.gitRepo, dto.gitBranch || 'main', dto.gitCommit).catch(err => {
        this.logger.error(`Deployment failed for ${saved.name}: ${err.message}`);
      });
    }
  ...
  async deployFromGit(id: string, gitRepo: string, gitBranch: string, gitCommit?: string): Promise<void> {
    const app = await this.findById(id);
    ...
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'autocodeflow-deploy-'));
    try {
      // Clone repository
      ...
      execSync('git clone --depth 1 --branch ' + gitBranch + ' ' + gitRepo + ' ' + tmpDir, {
        timeout: 120_000,
        stdio: 'pipe',
      });

      app.gitCommit = execSync('git -C ' + tmpDir + ' rev-parse HEAD', { encoding: 'utf-8' }).trim();
      ...
      if (fs.existsSync(manifestPath)) {
        ...
        if (manifest.tasks && Array.isArray(manifest.tasks) && this._taskService) {
          this.logger.log(`Auto-registering ${manifest.tasks.length} tasks from manifest.json`);
          for (const taskDef of manifest.tasks) {
            try {
              ...
              if (err?.message?.includes('already exists')) {
                this.logger.warn(`Task "${taskDef.id || taskDef.name}" already exists, skipping`);
              } else {
                this.logger.error(`Failed to register task "${taskDef.name}": ${err.message}`);
              }
            }
          }
        }
   ...
   */
  async syncTasksFromManifest(appId: string, manifestPath?: string): Promise<number> {

...
describe('SystemConfigService', () => {
  ...
  beforeEach(async () => {
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        { provide: getRepositoryToken(ConfigHistory), useFactory: mockHistoryRepo },
      ],
    }).compile();
  ...
  describe('upsert', () => {
    const baseDto = { key: 'k1', value: 'v1', valueType: 'string', isSecret: false };
    ...
    beforeEach(() => {
      ...
      repo.findOneBy.mockResolvedValueOnce(null).mockResolvedValueOnce({ key: 'k1', value: 'v1' } as SystemConfig);
    });
  ...
  describe('rollback', () => {
    ...
    it('should call upsert with oldValue for non-delete action', async () => {
      const history = { id: 1, configKey: 'k1', oldValue: 'old', action: 'update' };
      historyRepo.findOneBy.mockResolvedValue(history);
      // upsert needs its dependencies mocked
      repo.findOneBy.mockResolvedValueOnce({ key: 'k1', value: 'new' } as SystemConfig);
      repo.upsert.mockResolvedValue(undefined);
      historyRepo.create.mockImplementation((d) => d);
      historyRepo.save.mockResolvedValue({});
      repo.findOneBy.mockResolvedValueOnce({ key: 'k1', value: 'old' } as SystemConfig);
  ...
  describe('getHistory', () => {
    ...
    it('should filter by key when provided', async () => {
      ...
      expect(qb.where).toHaveBeenCalledWith('h.configKey = :key', { key: 'myKey' });
      expect(result.data).toEqual(data);
    });
  });
  ...
  describe('getByPrefix', () => {
    it('should query configs by prefix', async () => {
      ...
      expect(qb.where).toHaveBeenCalledWith('c.key LIKE :prefix', { prefix: 'app.%' });
      expect(result).toEqual(configs);
    });
  });
  ...
  describe('validateConfig', () => {
    ...
    it('should pass for valid boolean true', async () => {
      await expect(
        service.validateConfig({ key: 'k', value: 'true', valueType: 'boolean' }),
      ).resolves.toBeUndefined();
    });
  ...
  describe('batchUpsert', () => {
    it('should upsert each item and return results', async () => {
      ...
      historyRepo.save.mockResolvedValue({});
      repo.findOneBy
        .mockResolvedValueOnce(null).mockResolvedValueOnce({ key: 'k1', value: 'v1' } as SystemConfig)
        .mockResolvedValueOnce(null).mockResolvedValueOnce({ key: 'k2', value: 'v2' } as SystemConfig);
      const items = [
        { key: 'k1', value: 'v1' },
        { key: 'k2', value: 'v2' },
      ];

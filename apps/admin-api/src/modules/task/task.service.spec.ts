...
const mockRepo = () => ({ create: jest.fn(), save: jest.fn(), findOne: jest.fn(), findAndCount: jest.fn() });
...
describe('TaskService', () => {
  ...
  describe('trigger', () => {
    it('should create execution and enqueue job', async () => {
      const task = { id: '1', name: 'test', params: {}, maxRetry: 3, currentVersion: 'v1' };
      const exec = { id: 'exec-1', status: ExecutionStatus.PENDING };
      taskRepo.findOne.mockResolvedValue(task);
      dataSource.transaction.mockImplementation((fn: any) => fn({
      ...
      expect(taskQueue.add).toHaveBeenCalledWith('execute', { executionId: exec.id }, { attempts: task.maxRetry });

...
describe("UsersController", () => {
  ...
  describe("findOne", () => {
    ...
    it("non-admin cannot fetch another user", async () => {
      const self = { id: 1, role: "viewer" };
      await expect(controller.findOne(5, self as any)).rejects.toThrow();
    });
    it("returns ForbiddenException when user is missing (H-3)", async () => {
      usersSvc.findByIdOrNull.mockResolvedValue(null);
      await expect(controller.findOne(5, adminUser)).rejects.toThrow();
    });
  });
  ...
  describe("update", () => {
    ...
    it("non-admin rejected when currentPassword is wrong", async () => {
      ...
      await expect(
        controller.update(
          2,
          { password: "newpass", currentPassword: "wrong" } as any,
          normalUser,
          mockReq,
        ),
      ).rejects.toThrow(BadRequestException);

...
export class AlertmanagerAlertDto {
  @ApiPropertyOptional({ description: "告警状态 firing/resolved", example: "firing" })
  status?: string;

  @ApiPropertyOptional({ description: "告警标签（含 alertname/labels.taskId 等）", example: { alertname: "HighCPU", taskId: "uuid" } })
  labels?: Record<string, string>;

  @ApiPropertyOptional({ description: "告警注解（含 runbook_url 惯例）", example: { summary: "CPU > 90%" } })
  annotations?: Record<string, string>;

  @ApiPropertyOptional({ description: "开始时间（ISO 字符串原样透出）", example: "2026-09-14T06:00:00Z" })
  startsAt?: string;

  @ApiPropertyOptional({ description: "结束时间（resolved 时）", example: "2026-09-14T06:05:00Z" })
  endsAt?: string;

  @ApiPropertyOptional({ description: "告警生成器 URL", example: "http://grafana/alert/1" })
  generatorURL?: string;
...
export class AlertmanagerWebhookDto {
  @ApiPropertyOptional({ description: "Alertmanager webhook 协议版本", example: "4" })
  version?: string;
  ...
  @ApiPropertyOptional({ type: [AlertmanagerAlertDto], description: "告警列表（空数组被控制器 400 拒绝）" })

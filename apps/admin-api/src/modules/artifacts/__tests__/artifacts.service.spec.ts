...
describe("ArtifactsService（FEAT-05）", () => {
  ...
  afterAll(() => {
    ...
    if (originalLogArtifactDir === undefined) delete process.env.LOG_ARTIFACT_DIR;
  ...
  describe("resolveArtifactPath 防路径穿越", () => {
    ...
    it("拒绝含路径分隔符的名字", () => {
      expect(() => svc.resolveArtifactPath(EXEC_ID, "a/b.png")).toThrow(BadRequestException);
      expect(() => svc.resolveArtifactPath(EXEC_ID, "a\\b.png")).toThrow(BadRequestException);
    });
    it("拒绝前导点/绝对路径/空名", () => {
      expect(() => svc.resolveArtifactPath(EXEC_ID, ".hidden")).toThrow(BadRequestException);
      expect(() => svc.resolveArtifactPath(EXEC_ID, "/etc/passwd")).toThrow(BadRequestException);
      expect(() => svc.resolveArtifactPath(EXEC_ID, "")).toThrow(BadRequestException);
    });
    it("非 UUID executionId 抛 BadRequest", () => {
      expect(() => svc.resolveArtifactPath("not-a-uuid", "a.png")).toThrow(BadRequestException);
    });
  });
  ...
  describe("saveArtifact / openArtifact 落盘回环", () => {
    it("写入后可读回，字节/大小/sha 一致，并回显清单条目", async () => {
      ...
      expect(item).toEqual({ name: "report.csv", size: buf.length, sha256: sha(buf) });

      const { stream, fileSize, contentType } = await svc.openArtifact(EXEC_ID, "report.csv");
      const chunks: Buffer[] = [];
    ...
    it("sha256 不符抛 BadRequest 且不留脏文件", async () => {
      ...
      await expect(
        svc.saveArtifact(EXEC_ID, "tampered.bin", buf, sha(Buffer.from("other"))),
      ).rejects.toBeInstanceOf(BadRequestException);
      expect(fs.existsSync(path.join(tmpRoot, EXEC_ID, "tampered.bin"))).toBe(false);
    });
    it("openArtifact 缺文件抛 NotFound", async () => {
      await expect(svc.openArtifact(EXEC_ID, "missing.txt")).rejects.toBeInstanceOf(
        NotFoundException,
      );
    });
  });
  ...
  describe("verifyUploadAuth（复用回调鉴权形态）", () => {
    it("执行行不存在 → NotFound", async () => {
      ...
      await expect(svc.verifyUploadAuth(EXEC_ID, "Bearer x")).rejects.toBeInstanceOf(
        NotFoundException,
      );
    });
    it("共享 token 有效 → 放行", async () => {
      findOne.mockResolvedValue({ id: EXEC_ID, executorAddress: "1.2.3.4:9000" });
    ...
    it("共享无效但每执行器动态 token 有效 → 放行", async () => {
      ...
      expect(exec.id).toBe(EXEC_ID);
      expect(validateTokenByAddress).toHaveBeenCalledWith("1.2.3.4:9000", "dynamic");
    });
    it("两条凭据均失败 → Unauthorized", async () => {
      findOne.mockResolvedValue({ id: EXEC_ID, executorAddress: "1.2.3.4:9000" });
      (verifyExecutorToken as jest.Mock).mockRejectedValue(new Error("nope"));
      validateTokenByAddress.mockResolvedValue(false);
      await expect(svc.verifyUploadAuth(EXEC_ID, "Bearer bad")).rejects.toBeInstanceOf(
  ...
  describe("getManifest", () => {
    it("返回行内存清单，缺省为 []", async () => {
      findOne.mockResolvedValue({ id: EXEC_ID, artifacts: [{ name: "a.png", size: 1, sha256: "x" }] });

...
describe('安装向导第 5 步：只认本次新出现的执行器', () => {
  ...
  it('基线不可信时不得据此判定"安装成功"（P1-22 修复后的语义）', () => {
    // 修复前：handleGoToStep4 在 list() 失败时传空集合，而空集合意味着
    // "任何在线执行器都是新的"——在已有执行器的机队里，一次瞬时网络失败就会让
    // 第 5 步打绿勾并**印出另一台机器的名字**。而"网络不健康"恰恰是安装最可能
    // 失败的条件：误报成功比 60s 超时更糟（用户带着假结论离开）。
    //
    // 修复后：基线取不到就**不推进**（显示错误 + 允许重试）。本用例钉住"不可信
    // 基线不得被当成判据"这一语义：调用方必须先拿到基线才能调用本函数。
    // 反证：把 handleGoToStep4 的 catch 改回"传空集合继续"，页面级用例转红
    // （见 install-wizard-baseline.test.tsx）。
    const rows = [online('exec-anything')];
    // 空集合**在函数语义上**仍表示"基线为空"（纯函数不负责判定基线是否可信），
    // 但调用方已不再在失败路径上传它——该约束由页面级用例把守。

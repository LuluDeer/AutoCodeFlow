...
> 令牌缓存 Redis 化（3.7 🟡）、无 Leader 门禁的 @Cron（3.8 🟡）、快速路径重试
> pendingTimers（总表 #8 🟡），另多主机（跨机）拓扑尚留验。范围：`apps/admin-api/src`。
...
> 端到端验证；**本地文件系统**（需共享卷）仍是水平扩容的主要部署约束，令牌缓存
> （3.7）、无 Leader 门禁的 @Cron（3.8）等 🟡 项按 §5 建议继续加固。
>
...
- **建议加固**：`@Cron` 统一 Leader 门禁（3.8，可抽 `@LeaderOnly()` 装饰器复用
  scheduler 的 `isLeader`/Redis 锁，fail-open 语义一致）；outbox 行级 claim（4.1
  方案 A）已落地并经双实例验证（见 3.6）。
...
以上 1~4 为**代码实现完成 + 单测覆盖**；真机双实例端到端验证已按清单逐项执行完毕——清单 5/5 ✅（1/2/5 于 2026-09-12、3 于 2026-09-12 补充、4 的重复投递端到端闭环于 2026-09-13），多主机（跨机）拓扑留验。

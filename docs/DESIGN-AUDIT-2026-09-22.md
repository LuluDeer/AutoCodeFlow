...
| # | 级别 | 条目 | 状态 | commit |
|---|---|---|---|---|
| D-P1-1 | P1 | 任务列表调度列 fixed_rate≥60s 二次包裹出「每 2 分钟 秒」 | ⬜ | — |
| D-P1-2 | P1 | 剪贴板诚实性守卫缺口：AppDeploymentPage 静默 catch、ExecutionDetailPage 无 fallback | ⬜ | — |
| D-P2-01 | P2 | 全仓 Alert `message=` 迁移残留（~12 处）统一为 `title=` | ⬜ | — |
| D-P2-02 | P2 | 依赖图/详情页 runtime、status 等裸枚举未本地化 | ⬜ | — |
| D-P2-03 | P2 | timeout=0（不限时）在详情页显示 `-` | ⬜ | — |
| D-P2-04 | P2 | 仪表盘 SSE 状态点色硬编码 hex，未走语义令牌 | ⬜ | — |
| D-P2-05 | P2 | 依赖图节点可点击但无键盘可达 | ⬜ | — |
| D-P2-06 | P2 | 依赖图当前节点阴影硬编码旧 antd 蓝 | ⬜ | — |
| D-P2-07 | P2 | 依赖图整 Tab 加载用裸居中 Spin | ⬜ | — |
...
| D-P2-16 | P2 | 品牌渐变内联重复 5 处，未收 token | ⬜ | — |

...
> **2026-09-15 更新（TASK-PROJ-01，产品已拍板）**：本条曾有一个未兑现的收尾项。
> 迁移 `1790000000008` 给 `tasks` 加了 `projectId` 列并回填了**存量**任务，但其注释
> 写明「新建任务在 DTO 未接 projectId 前一律落 NULL」——而该 DTO 字段一直没做。
> 后果：**新任务永远 `projectId = NULL`**，于是本节「NULL → 默认项目」的塌缩对
> 全部新建任务成立，「项目隔离」事实上形同虚设（这正是缺口②的根因）。
>
> 现已补齐写入链路：
> - `CreateTaskDto.projectId`（`@IsUUID` 可选；省略/null = 未分配，与既有行为一致）；
> - 写面校验 `TaskService.assertCanAssignProject`：项目须**真实存在**（否则 400，
>   而不是让 FK 违例冒成 500），且调用方须为 ADMIN 或该项目 editor/admin
>   （否则任何人都能把任务塞进/捞出别人的项目）；
> - `update` 同样校验（缺省 = 保留旧归属，不重复校验）；
> - 前端任务表单新增「归属项目」选择器；编辑态回填。
>
> 仍是**纯增量**：不传 `projectId` 的既有调用方（含全部 CLI/SDK/MCP）行为逐字节不变。

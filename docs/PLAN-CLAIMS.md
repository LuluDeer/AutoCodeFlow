...
| OBS-02 | P1 | in_progress | 001（子代理） | 2026-09-07 13:0x | admin-api notification 模块（新 alerts.controller + alert-webhook.service + spec）+ app.module/configuration/.env.example（ALERT_WEBHOOK_SECRET）+ docs/observability（Alertmanager 配置样例）+ docs/api-reference.md（新端点段，小改即提交）。零迁移（taskId 查 tasks.runbook 为只读复用既有表，不占时间戳） | | 告警路由到通知渠道（依赖 FEAT-11 已就绪） |

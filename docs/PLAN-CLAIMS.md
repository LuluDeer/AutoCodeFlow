...
| CORE-02 | P1 | in_progress | 002（子代理） | 2026-09-07 12:36 | admin-api task 模块（retryableErrors DTO 校验+jitter util+spec）+ admin-web TaskFormPage/ExecutionDetailPage + docs/api-reference.md（预期不新增迁移；若需迁移占用 1789700000000） | | 重试策略精细化（attempt 链可视化） |
...
| OBS-04 | P1 | in_progress | 001（子代理） | 2026-09-07 12:41 | admin-api task 模块（report 读端点+独立 spec）+ admin-web ExecutionDetailPage（最小 Tab 插入）+ 新组件文件（时间线/报告面板+测试，独立文件零重叠）+ api/execution-reports.ts（新建，避开 002 的 api/tasks.ts）+ docs/api-reference.md。⚠️ 发现 002/CORE-02 足迹含 ExecutionDetailPage——已降级为最小插入+新文件策略，核心产物全在独立文件；TaskDetailPage/artifacts 文件属 003 在途，勿动 | | execution_reports 消费 + 时间线 Tab |

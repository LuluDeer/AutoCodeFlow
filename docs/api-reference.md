...
- **SEC-05 zip bomb 防护（400 拒绝）**：包落库前过中央目录结构守卫——解压比 / 条目数 / 单文件与总量声明上限（env `ZIP_MAX_*` 可调，默认 100 / 10000 / 512 MiB / 1 GiB / 嵌套成员 64 MiB）+ 嵌套 zip 探测 1 层；结构损坏（截断 / CD 尺寸篡改 / zip64 哨兵）同样 400（`Package rejected by zip-bomb guard (<violation>)`）
...
| `secrets` | object | 否 | 任务级凭据键值对（SEC-02，独立于 `params` 的普通运行参数）。**存储加密**：配置 `SEC_SECRETS_KEY` 后所有叶子值以 AES-256-GCM `enc:v1:<iv>:<tag>:<ciphertext>` 信封落库；未配置时降级明文并启动 warn 一次（零破坏升级路径）。**读取永久脱敏**：`GET /tasks`、`GET /tasks/:id` 响应中叶子值一律回 `******`（密文也不外泄），因此已保存的 secrets 不可经 API 回读。**派发语义（SEC-02 续）**：执行时解密后走**两条**通道——① 与 params 合并注入 `AUTOFLOW_<KEY>`（旧执行器只认这条，兼容红线）；② 作为载荷的独立 `secrets` 字段下发，由执行器按**原名**注入子进程 env（脚本写 `os.environ["FEISHU_APP_ID"]`、boto3 认 `AWS_ACCESS_KEY_ID` 等第三方 SDK 规范名即可直接读到）。凭据名称须匹配 `[A-Za-z_][A-Za-z0-9_]*` 且不得占用保留名（`PATH` / `PYTHON*` / `AUTOFLOW_*` 等），否则保存即 400。明文仅存在于派发 HTTPS 载荷与执行器内存，不落 `task_executions.params`。**PATCH 语义（逐键合并，非整体替换）**：字段缺省 = 全部保留；显式 `null` = 清空全部；对象内某键缺省 = 该键保留；叶子值 = `******` = 该键保留（读路径回传的掩码即"不改"）；叶子值 = `null` = 删除该键；其它值 = 加密覆盖。之所以必须逐键合并：读路径永久脱敏，控制台要显示既有键就必然持有掩码值，整体替换会把掩码当真实凭据落库（真实值不可逆损毁，而任务仍报"缺少凭据"）。存量行不做迁移加密——配置 key 后首次 update 自然转为密文 |
...
| 方法 | 路径 | 需要认证 | 说明 |
|------|------|:--------:|------|
| POST | `/executors/register` | 否* | 执行器注册（返回/绑定执行器专属 Token）。可选字段：`protocolVersion`（整数，协议版本）/ `dispatchMode`（`push`/`pull`）/ `interpreters`（解释器缓存池清单）/ `deviceFingerprint`（**协议 v3 起**，见心跳行说明） |
| POST | `/executors/heartbeat` | 否* | 执行器心跳上报（携带 `restartedAt`/`startupId` 用于重启收敛）。可选指标字段：`cpuUsage` / `memUsage` / `diskUsage` / `networkLatency` / `runningTaskCount`（**已占槽位**，含 E-01 取件预留）/ `totalTaskCount` / `failedTaskCount` / `runningExecutionIds`（≤10000，`null`=旧版未上报；**在跑执行**的 id 清单，与 `runningTaskCount` 是两个账本）/ `reservedSlots`（**E-01-RPT，协议 v4 起**：pull 长轮询「已预留但尚未认领」的槽位数，非负整数且必须 `≤ runningTaskCount`（预留是已占槽位的子集，越界即拒绝采纳、库内原值不动）；**始终发送含 `0`**，`null`=旧版执行器未上报。**只用于展示与告警换算**：详情页据此显示「实际运行 = runningTaskCount − reservedSlots」并与 `runningExecutionIds.length` 比对，派发闸门仍只读 `runningTaskCount`——否则会重开 E-01 要关闭的超卖竞态）/ `deadLetterCount`（回调死信积压数，**0..100000** 非负整数，`null`=旧版执行器未上报该字段（区别于 `0`：已上报且无积压），`>0` 表示回调持续失败、载荷已落盘执行器本地 dead-letter）/ `maxConcurrentTasks`（1..10000 容量热更新）/ `deviceFingerprint`（**ARCH-36 / ADR-017 阶段 2，协议 v3 起**：稳定安装身份 `sha256(deviceId + ":" + installSalt)` 的 **64 位小写十六进制**。**只采集与观测，不参与任何定位**（注册仍按 `address` 定位行）；缺省或形态非法一律**保留库内原值**——旧执行器或采集失败的执行器绝不会把已存指纹擦成 `null`，故 `null` 恒表示「从未上报过」）。服务端对白名单外字段静默丢弃（防 mass-assignment），非法取值不落库 |

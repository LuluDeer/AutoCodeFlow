...
#### 8.2.2 服务端水位（本轮补上 RSS/CPU 采样）

`load-test-stack.sh` 现在自带**服务端水位采样**（每 2s 读 admin-api 进程
`/proc/<pid>/status` + `/proc/<pid>/stat`，产出峰值 RSS 与窗口平均 CPU；不依赖
Prometheus scrape，也无需鉴权），随压测报告一起打印。

| 档位 | admin-api 峰值 RSS | 平均 CPU（单核百分比） | 采样 |
|---|---|---|---|
| 500 并发执行（100% 成功、p95 26ms） | **403 MB** | **6.59%** | 91 次 / 2s 间隔 |

- **读法**：500 并发在途 + 100% 成功率下，admin-api 只吃掉约**单核 6.6%**，
  说明瓶颈既不在 CPU 也不在内存，而在客户端预算/轮询节奏（见口径说明）。
- **仍缺**：PG 连接池占用、BullMQ 队列深度、事件循环延迟（需 Prometheus scrape
  侧或 DB/Redis 探针），以及「经反代的 500 连接档」与「多实例槽位分布」。

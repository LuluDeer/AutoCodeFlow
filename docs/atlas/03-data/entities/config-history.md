...
`config_history`（`@Entity("config_history")`，InitialSchema 迁移 `1717473142678` 建表；`action` 扩值由迁移 `1789000000001` 承载）
...
- **读取**：历史列表（按 configKey 分页过滤，`config-history-query.dto.ts`）、回滚端点（读 oldValue/newValue 还原，且回滚动作本身再落 `action=rollback` 行）。
- **清理**：无自动清理；长期增长需人工归档。
